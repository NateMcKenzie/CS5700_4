...
class ConvertBase10(private val registerBank: RegisterBank) : Instruction(registerBank) {
    override fun mainFunction(nibbleds: Pair<Nibbled, Nibbled>) {
        ...
        val ones = value % 10u
        D5700.memory.write(shortToBytes(memoryAddress), hundreds.toUByte())
        D5700.memory.write(shortToBytes((memoryAddress+1u).toUShort()), tens.toUByte())
        D5700.memory.write(shortToBytes((memoryAddress+2u).toUShort()), ones.toUByte())
    }
}